...
export default function RangeSummary({ days, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        <Stat label="Submissions" value={totalSub} />
        <Stat label="Accepted" value={totalAcc} />
        <Stat label="Solved" value={totalSolved} />
        <Stat label="Hard" value={totalHard} />
        <Stat label="Acceptance" value={`${accRate}%`} />
        <Stat label="Time" value={hours > 0 ? `${hours}h ${mins}m` : `${mins}m`} />
      </div>
    </div>
  );
}
